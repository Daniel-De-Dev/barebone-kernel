...
bootloader_image="${MANGOPI_BOOTLOADER_IMAGE:?MANGOPI_BOOTLOADER_IMAGE must be set by Nix}"
bootloader_address="${MANGOPI_BOOTLOADER_ADDRESS:?MANGOPI_BOOTLOADER_ADDRESS must be set by Nix}"
...
Boot the bootloader on a MangoPi MQ Pro through FEL.
...
xfel ddr d1
echo "Uploading bootloader to ${bootloader_address}..."
xfel write "${bootloader_address}" "${bootloader_image}"
